...
export async function POST(
  ...
  if (!workspace.agentPort) {
    return NextResponse.json({ error: 'Workspace not running' }, { status: 400 });
  }

  try {
    // Send command to VSCode extension to start Live Share
    const response = await fetch(`http://localhost:${workspace.agentPort}`, {
      method: 'POST',
    ...
    if (!response.ok) {
      throw new Error('Failed to communicate with workspace');
    }
    ...
    return NextResponse.json({
      success: true,
      shareLink: data.data?.shareLink || 'https://vscode.dev/liveshare/join/session-id',
    });
  } catch (error: any) {
...
export async function DELETE(
  ...
  try {
    await fetch(`http://localhost:${workspace.agentPort}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
